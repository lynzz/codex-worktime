...
describe("persistent attribution overrides", () => {
  it("appends original evidence and revocations without rewriting prior records", async () => {
    ...
    const storage = { databasePath, applicationDataDirectory: databasePath.slice(0, databasePath.lastIndexOf("/")), projectRoots: ["/private/eqa"] };
    appendOverride(storage, { id: "override", attribution: original, replacementFeatureId: "invoicing", reason: "Reviewed" });
    revokeOverride(storage, "override", "Reversed");

    expect(listOverrides(storage)).toEqual([
