  ...
  <body>
    <main>
      ...
      <section class="panel"><h2>{% if view === "internal" %}已核验区间、提交节奏与数据覆盖{% else %}已核验区间与数据覆盖{% endif %}</h2>
        <h3>按日活跃、提交与覆盖情况（Asia/Shanghai）</h3>{% if view === "internal" %}<p class="panel-note">提交节奏推测只累计相邻且同一 scope 的提交间隔，每段最多 60 分钟；它不是已核验 AI 或人工工时。</p>{% endif %}<table class="detail-table"><thead><tr><th>日期</th><th>已核验活跃</th><th>提交历史汇总</th>{% if view === "internal" %}<th>提交节奏推测（非核验）</th>{% endif %}<th>无数据与覆盖情况</th></tr></thead><tbody>{% for entry in dailyRows %}<tr{% if entry.coverageLabel %} aria-label="{{ entry.date }}: {{ entry.coverageLabel }}"{% endif %}><td>{{ entry.date }}</td><td class="number">{{ entry.activeLabel }}</td><td>{% if entry.commitCount %}<strong>{{ entry.commitCount }} 个提交</strong><br><span class="muted">{{ entry.commitSummary }}</span>{% else %}<span class="muted">—</span>{% endif %}</td>{% if view === "internal" %}<td>{% if entry.commitEstimateMinutes %}<strong class="number">{{ entry.commitEstimateMinutes }} 分钟</strong><br><span class="muted">{{ entry.commitEstimateSummary }}</span>{% else %}<span class="muted">—</span>{% endif %}</td>{% endif %}<td>{{ entry.coverageLabel or "无记录（不代表零工时）" }}</td></tr>{% else %}<tr><td colspan="{% if view === \"internal\" %}5{% else %}4{% endif %}" class="muted">本周期没有已核验的活跃区间、提交记录或覆盖记录。</td></tr>{% endfor %}</tbody></table>
...
export async function generateProjectReport(input: GenerateProjectReportInput): Promise<ProjectReportResult> {
  ...
  const commitReportData = dateRange
    ...
    : { estimates: [], dailySummaries: [], dailyEstimates: [] };
